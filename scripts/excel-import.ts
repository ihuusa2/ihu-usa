...
interface ImportConfig {
    ...
    transformData?: (row: ExcelRow) => any;
...
const importConfigs: { [key: string]: ImportConfig } = {
    registration: {
        ...
        transformData: (row: ExcelRow) => ({
            ...
            createdAt: new Date(),
            recieved: {
                diploma: Boolean(row['Diploma']),
                homeSchool: Boolean(row['Home School']),
                ged: Boolean(row['GED']),
    ...
    users: {
        ...
        transformData: async (row: ExcelRow) => {
            const hashedPassword = await bcrypt.hash(row.password || 'defaultPassword123', 10);
            return {
                ...row,
...
class ExcelImporter {
    ...
    mapFields(row: ExcelRow, fieldMapping: { [excelField: string]: string }): any {
        const mapped: any = {};
        ...
        for (const [excelField, dbField] of Object.entries(fieldMapping)) {
            if (row[excelField] !== undefined && row[excelField] !== null && row[excelField] !== '') {
                ...
                if (dbField.includes('.')) {
                    ...
                    mapped[parent][child] = row[excelField];
                } else {
    ...
    async importCollection(config: ImportConfig): Promise<{ success: number; errors: any[] }> {
        ...
        try {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                try {
                    ...
                    await collection.insertOne(mappedData);
                    results.success++;
